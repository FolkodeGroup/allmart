...
export async function login(username: string, password: string): Promise<LoginResult> {
  ...
  // Aseguramos que el usuario exista y tenga una contraseña asignada
...
export async function changePassword(
  ...
  // Aseguramos que el usuario exista y tenga una contraseña asignada
  if (!user || !user.passwordHash) {
    throw Object.assign(new Error('La contraseña actual es incorrecta o el usuario no existe'), { statusCode: 400 });
  }
